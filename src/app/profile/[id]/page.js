      ...
      try {
        ...
        if (isOwner) {
          ...
          if (currentUser?.is_creator) {
            ...
            if (kycRes.ok) setKycData(await kycRes.json());
          }
        } else {
              ...
              <div className="space-y-6">
                ...
                {isOwner && (
                  <div className="flex items-center justify-between p-3 rounded-2xl bg-indigo-600/5 border border-indigo-500/10">
                    ...
                    {kycData?.status !== 'approved' && kycData?.status !== 'pending' && (
                      <Link href="/kyc" className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-black uppercase tracking-widest rounded-lg transition-all shadow-lg shadow-indigo-500/20 active:scale-95">
                        Verify
                      </Link>
                    )}
                  </div>
