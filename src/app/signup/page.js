...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Backend unreachable — demo mode: simulate signup
      dispatch(loginSuccess({
        access: 'demo_access_token',
        refresh: 'demo_refresh_token',
        user: { email: formData.email, username: formData.username, id: 1 }
      }));
      router.push('/');
    } finally {
      setIsLoading(false);
    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setError('Google signup failed at backend. Please try again.');
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-[#0e0f11]">
      ...
      <div className="w-full max-w-md p-8 md:p-10 z-10 bg-black/40 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl relative">
        ...
        <p className="text-gray-400 text-center text-sm md:text-base mb-8">The next wave in music discovery.</p>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" name="username" value={formData.username} onChange={handleChange} required className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-3.5 text-white outline-none focus:border-indigo-500/50 transition-colors" placeholder="johndoe" />
            </div>
            <div className="flex flex-col gap-1.5">
        ...
        <p className="text-center text-gray-400 text-sm mt-8 uppercase tracking-widest font-bold">
          Been here before? <Link href="/login" className="text-white hover:text-indigo-400 transition-colors ml-1">Log in</Link>
        </p>
      </div>
    </div>
  );
